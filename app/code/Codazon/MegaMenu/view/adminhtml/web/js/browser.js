    ...
    CdzMediabrowserUtility = {
        ...
        openDialog: function(url, width, height, title, options) {
            ...
            $.ajax({
                ...
                self.modal.html(data.replace('"mediabrowser"', '"Codazon_MegaMenu/js/browser"')).trigger('contentUpdated');
            });
        },
        ...
        insert: function(event) {
            ...
            return $.ajax({
                ...
                if (typeof data == 'string') {
                    ...
                    if (!data.short) {
                        img.gsub(/src="(.*?)"/i, function (match) {
                            var src = '';
                            data.full = match[1];
                            data.short = '{{media url="' + match[1].replace(Codazon.mediaUrl, '') + '"}}';
                        });
                    }
                }
