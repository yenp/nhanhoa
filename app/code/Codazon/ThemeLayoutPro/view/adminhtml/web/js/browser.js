...
define([
    ...
    if(typeof window.CdzMediabrowserUtility === 'undefined') {
        window.CdzMediabrowserUtility = {
            ...
            openDialog: function(btn, width, height, title, options) {
                var self = this, conf = this.options;
                this.targetParent = $(btn).parents(conf.parent).first();
                this.targetElement = this.targetParent.find(conf.targetElement).first();
                ...
                if (conf.imageUrl) {
                    ...
                    if (!this.modal) {
                        ...
                        $.ajax({
                            ...
                            self.modal.html(data.replace('"mediabrowser"', '"Codazon_ThemeLayoutPro/js/browser"')).trigger('contentUpdated');
                        });
                    } else {
                        this.modal.modal('openModal');
                    }
    ...
    $.widget("codazon.cdzMediabrowser", {
        ...
        insert: function(event) {
            ...
            return $.ajax({
                ...
                if (typeof data == 'string') {
                    ...
                    data = {};
                    data.short = jQuery(img).attr('src');
                    data.full = Codazon.mediaUrl + data.short.gsub(/\{\{media(.*?)\}\}/i, function (match) {
                        var src = '';
                ...
                if (targetEl.is('textarea')) {
                    ...
                    if (typeof CdzMediabrowserUtility.targetParent !== 'undefined') {
                        CdzMediabrowserUtility.targetParent.find('[data-role=image-preview]').attr('src', data.full);
                    }
	...
	return $.codazon.cdzMediabrowser;
});